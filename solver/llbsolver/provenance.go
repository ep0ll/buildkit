...
func (b *provenanceBridge) Solve(ctx context.Context, req frontend.SolveRequest, sid string) (res *frontend.Result, err error) {
	...
	// Choke point: if the request carries a FilteredSession (a *FilteredManager),
	// inject it into the builder so that every op resolved for this nested build
	// receives a *FilteredCaller from jobCtx.CallerManager() — no context values
	// are used; enforcement is at the gRPC transport layer inside FilteredCaller.
	if req.FilteredSession != nil {
		b.builder.SetCallerManager(req.FilteredSession)
	}
